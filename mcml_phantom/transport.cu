	...
	for(int i = 0; i < NUM_THREADS; i++)
	{
		...
		else
		{
			...
			*/
			...
			result[7] += f[i].data[8];
			result[8] += f[i].data[9];
			result[9] += f[i].data[10];
			result[10] += f[i].data[11];
			result[11] += f[i].data[12];
			*/
...
__device__ void LaunchPhoton(PhotonStruct* p, curandState *state)
{
	...
	launchPosition = beam_width*sqrt(-log(rn_gen(state))/2.0); // YU-modified

	p->x = launchPosition*cos(AzimuthAngle) / cos(angle); //p->x = 0.0;	 // YU-modified
	p->y = launchPosition*sin(AzimuthAngle);              //p->y = 0.0; // YU-modified
	p->z = 0.0;
	...
	p->dx = uxprime, p->dy = uyprime, p->dz = uzprime;//p->dx = 0.0, p->dy = 0.0, p->dz = 1.0; // //YU-modified
