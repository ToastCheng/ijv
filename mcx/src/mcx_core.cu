...
__device__ inline void savedetphoton(float n_det[],uint *detectedphoton,float nscat,float *ppath,MCXpos *p0,MCXdir *v,RandType t[RAND_BUF_LEN],RandType *seeddata){
      ...
      if(detid){
        ...
        if(baseaddr<gcfg->maxdetphoton){
          ...
          baseaddr*=gcfg->maxmedia+2+gcfg->issaveexit*6+(gcfg->ismomentum*gcfg->maxmedia)+1;
...
void mcx_run_simulation(Config *cfg,GPUInfo *gpu){
     ...
     // [toast note] here to set the data form
     // int detreclen=cfg->medianum+1+(cfg->issaveexit>0)*6+((cfg->ismomentum>0)*(cfg->medianum-1));
     // [toast modified] add one row to save weight
     int detreclen=cfg->medianum+1+(cfg->issaveexit>0)*6+((cfg->ismomentum>0)*(cfg->medianum-1))+1;
