...
void mcx_loadconfig(FILE *in, Config *cfg){
	...
	cfg->detpos[i].w=cfg->detradius;
     ...
     // [toast modified] add weight
     cfg->his.colcount=cfg->medianum+1+(cfg->issaveexit)*6+((cfg->ismomentum>0)*(cfg->medianum-1))+1; /*column count=maxmedia+2*/
...
###############################################################################\n\
$Rev::e37fc5 $ Last $Date::Qianqian Fang          $ by $Author::Qianqian Fang $\n\
###############################################################################\n");
}
